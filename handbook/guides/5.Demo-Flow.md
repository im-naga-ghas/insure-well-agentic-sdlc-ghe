...
On first run, the application starts with a **Flask** backend and a **SQLite** database seeded with sample policies and claims.

**What you see:**
- Policy management and lifecycle status views
- Claims submission, policy-based filtering, and status tracking (Pending / Approved / Rejected)
...
| Agent file | Role |
|---|---|
| `sdlc-hld-agent.md` | High-Level Design — architecture, component boundaries, data flow |
| `sdlc-brd-agent.md` | Business Requirements — BRD, epics, and features |
| `sdlc-dev-agent.md` | Feature implementation — React + Flask delivery |
| `cloud-agent.md` | Cloud/infra — IaC, deployment configs, environment setup |
| `sdlc-test-agent.md` | Quality Assurance — test generation, CI triage, coverage |
...
**Expected output:**
- Architecture overview (React frontend, Flask backend, SQLite persistence)
...
- API surface map aligned with Flask routes defined in `app.py` (for policy and claim endpoints)
...
```
@sdlc-brd-agent Analyze the InsureWell application ([repository root](../../../), [README.md](../../../README.md), [docs/InsureWell_HLD.md](../../../docs/InsureWell_HLD.md)).
...
```
@sdlc-dev-agent Implement user authentication for InsureWell (Story #1).
- Add Flask-based login/authentication support for the backend (session or token-based)
- Introduce a users model and persistence strategy appropriate for SQLite (for example via SQLAlchemy)
