...
SEED = {
    ...
    'claims': [
        ...
        },
        {
            ...
            'description': 'Knee surgery – arthroscopic procedure',
            'status': 'Pending',
            'file_name': 'surgery_discharge_summary.pdf',
            'submitted_at': '2024-04-18T08:00:00.000Z',
            'updated_at': '2024-04-18T08:00:00.000Z',
        },
        {
...
def claims_page():
    policy_filter = request.args.get('policy_id', '')
    if policy_filter:
        filtered = [c for c in store['claims'] if c['policy_id'] == policy_filter]
    else:
        filtered = store['claims']
    filtered = sorted(filtered, key=lambda c: c['submitted_at'], reverse=True)
...
@app.route('/api/policies')
def api_get_policies():
    return jsonify(store['policies'])


@app.route('/api/policies/<pid>')
def api_get_policy(pid):
    policy = next((p for p in store['policies'] if p['id'] == pid), None)
    if not policy:
        return jsonify({'error': 'Policy not found'}), 404
...
def api_get_claims():
    pid = request.args.get('policy_id')
    result = [c for c in store['claims'] if c['policy_id'] == pid] if pid else list(store['claims'])
    result.sort(key=lambda c: c['submitted_at'], reverse=True)
...
def api_create_claim():
    ...
    if not any(p['id'] == policy_id for p in store['policies']):
        return jsonify({'error': 'Policy not found'}), 404
...
def api_update_claim_status(cid):
    ...
    claim = next((c for c in store['claims'] if c['id'] == cid), None)
    if not claim:
        return jsonify({'error': 'Claim not found'}), 404
